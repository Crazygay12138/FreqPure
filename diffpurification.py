...
class DiffPurificationForward(torch.nn.Module):
	def __init__(self, clf, diffusion, strength, is_imagenet, ddim_steps, amplitude_cut_range, phase_cut_range, delta,
	             device):
		super().__init__()
		self.clf = clf
	...
	def compute_fft(self, image):
		...
		for channel in range(3):
			f = torch.fft.fft2(image[channel, :, :])  # ⚠️
			fshift = torch.fft.fftshift(f)  # ⚠️
			amplitude = torch.abs(fshift)  # ⚠️
			phase = torch.angle(fshift)  # ⚠️
	...
	def low_pass_exchange(self, amplitude_channels, amplitude_channels_0_t):
		...
		for i in range(3):
			...
			frequency_map = torch.sqrt(U ** 2 + V ** 2)
	...
	def phase_low_pass_exchange(self, phase_channels, phase_channels_0_t):
		...
		for i in range(3):
			...
			phase_channels_0_t[i][low_frequency] = torch.clip(phase_channels_0_t[i][low_frequency],
	...
	def generate_frequency_exchange_matrix(self, rows, cols):
		matrix = torch.zeros((rows, cols), dtype=bool)
		frequency = int(1 / self.ratio)
		indices = torch.arange(0, rows * cols, frequency)
		for idx in indices:
			row = idx // rows
			col = idx % cols
			matrix[row, col] = True
	...
	def generate_random_exchange_matrix(self, rows, cols):
		...
		indices = torch.random.choice(1024, size=exchange_num, replace=False)
		for idx in indices:
			row = idx // rows
			col = idx % cols
			matrix[row, col] = True
	...
	def phase_clip(self, phase_channels, phase_channels_0_t, delta=0.6):
		phase_channels_clip = []
		for i in range(3):
			phase_channels_clip.append(
				torch.clip(phase_channels_0_t[i], phase_channels[i] - delta, phase_channels[i] + delta))
	...
	def reconstruct_image(self, filtered_amplitude_channels, phase_channels):
		...
		for channel in range(3):
			...
			f_ishift = torch.fft.ifftshift(fshift_filtered)  # ⚠️
			img_reconstructed = torch.fft.ifft2(f_ishift)  # ⚠️
			img_reconstructed = torch.abs(img_reconstructed)  # ⚠️

			img_reconstructed = torch.clip(img_reconstructed, 0, 255)
			reconstructed_image.append(img_reconstructed / 255)

		return torch.stack(reconstructed_image, dim=0)  # ⚠️

	def amplitude_phase_exchange(self, x, x_0_t, t):

		xx = diff2clf(x) * 255  # ⚠️
		xx_0_t = diff2clf(x_0_t) * 255  # ⚠️
		batch, channel, height, width = xx.shape
		new_x_0_t = torch.zeros_like(xx)
		for batch_idx in range(batch):
			amplitude_channels, phase_channels = self.compute_fft(xx[batch_idx])

			amplitude_channels_0_t, phase_channels_0_t = self.compute_fft(xx_0_t[batch_idx])
	...
	def oneStep(self, x, x_t, t, tau):  # ⚠️
		n = x_t.shape[0]
		if not math.isclose(self.eta, 0.0):
			one_minus_alpha_prod_tau = 1.0 - self.alphas_cumprod[tau]
			one_minus_alpha_prod_t = 1.0 - self.alphas_cumprod[t]
		...
		epsilon = torch.randn_like(x_t)
		return first_term + second_term + sigma_t * epsilon

	def denoiseGradCkpt(self, x):  # ⚠️
		x_t = self.diffuse_t_steps(x, self.timesteps[0])
		for t, tau in list(zip(self.timesteps[:-1], self.timesteps[1:])):
			x_t = checkpoint(self.oneStep, x, x_t, t, tau, preserve_rng_state=True)
		x_0 = x_t
		return x_0
	...
	def forward(self, x):
		...
		if self.gc:  # ⚠️
			x_diff = self.denoiseGradCkpt(x_diff)  # ⚠️
		else:  # ⚠️
			x_diff = self.denoise(x_diff)  # ⚠️
